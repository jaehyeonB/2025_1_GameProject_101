...
public class DragDrop : MonoBehaviour                   //카드 드래그 & 드롭 처리를 위한 클래스
{
    ...
    private void OnMouseUp()                            //마우스 버튼 놓을 때
    {
        isDragging = false;
        GetComponent<SpriteRenderer>().sortingOrder = 1;

        if (gameManager == null)                        //매니저가 없으면 함수를 종료한다
        {
        ...
        if(IsOverArea(gameManager.handArea))                                        //손패 영역 위에 카드를 놓았는지 확인
        {
            ...
            if(wasInMergeArea)                                                      //머지 영역에서 왔다면 MoveToHand 함수 호출
            {
                for(int i = 0; i < gameManager.mergeCount; i++)                     //카드를 머지 영역에서 제거하고 손패로 이동
                {
                    if (gameManager.mergeCards[i] == gameObject)                    //핸드 배열과 내가 마우스 업하는 오브젝트와 같은 경우
                    {
                        ...
                        gameManager.mergeCards[gameManager.mergeCount - 1] = null;          //맨 뒤에 카드를 null 로 설정
        ...
        {
            ReturnToOriginalPosition();                                             //아무 영역도 아닐 경우에는 원래 위치로 이동
        }
